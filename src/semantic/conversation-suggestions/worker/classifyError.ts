...
export type ConversationSuggestionErrorClassification =
...
export function classifyConversationSuggestionError(
  error: unknown,
): ConversationSuggestionErrorClassification {
  const summary = summarizeError(error);

  if (error instanceof ConversationSuggestionPermanentError) {
    return { kind: "permanent", summary };
  }

  if (error instanceof ConversationSuggestionTransientError) {
    return {
      kind: "transient",
      summary,
