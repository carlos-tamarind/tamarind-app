...
export type PageSemanticErrorClassification =
...
export function classifyPageSemanticError(error: unknown): PageSemanticErrorClassification {
  const summary = summarizeError(error);

  if (error instanceof PageSemanticPermanentError) {
    return { kind: "permanent", summary };
  }

  if (error instanceof PageSemanticTransientError) {
    return {
      kind: "transient",
      summary,
  ...
  if (
    /invalid|malformed|unexpected|unsupported|impossible|corrupt|missing required/i.test(
      summary,
    )
  ) {
    return { kind: "permanent", summary };
