-- ILIKE-based match detection with trigram similarity scoring.
-- Existing pg_trgm GIN indexes (idx_pages_title_trgm, idx_pages_content_trgm,
-- idx_conversations_title_trgm, idx_messages_normalized_trgm,
-- idx_workspace_users_display_name_trgm) already accelerate ILIKE '%...%'
-- for patterns of 3+ chars; no new indexes are required.
...
AS $$
  WITH scored AS (
    ...
    WHERE p.workspace_id = p_workspace_id
      AND (
        p.title ILIKE '%' || public.escape_ilike_pattern(p_query) || '%' ESCAPE '\'
        OR p.plain_text ILIKE '%' || public.escape_ilike_pattern(p_query) || '%' ESCAPE '\'
      )
  )
  SELECT
    s.id AS asset_id,
...
AS $$
  WITH scored AS (
    ...
    WHERE c.workspace_id = p_workspace_id
      ...
      AND c.title ILIKE '%' || public.escape_ilike_pattern(p_query) || '%' ESCAPE '\'
  )
  SELECT
    s.id AS asset_id,
    s.title,
...
AS $$
  WITH scored AS (
    ...
    WHERE c.workspace_id = p_workspace_id
      ...
      AND wu.display_name ILIKE '%' || public.escape_ilike_pattern(p_query) || '%' ESCAPE '\'
  ),
